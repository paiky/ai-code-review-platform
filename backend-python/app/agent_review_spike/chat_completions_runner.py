...
SAFE_ERROR_MESSAGES = {
    ...
    "AGENT_CHAT_COMPLETIONS_UNSUPPORTED": "中转站不支持 Chat Completions 协议",
    "AGENT_CHAT_COMPLETIONS_PROTOCOL_INVALID": "中转站返回的 Chat Completions 协议结构无效",
...
class ChatCompletionsAgentError(RuntimeError):
    def __init__(self, code: str) -> None:
        self.code = code
        super().__init__(SAFE_ERROR_MESSAGES.get(code, "Agent Review 执行失败"))


class ChatCompletionsTransportError(RuntimeError):
    def __init__(self, *, status_code: int | None = None) -> None:
        self.status_code = status_code
        super().__init__("Chat Completions transport failed")
...
class HttpxChatCompletionsTransport:
    """HTTP adapter that never returns provider error bodies to the runner."""
    ...
    def create(self, payload: dict[str, Any], timeout_seconds: float) -> dict[str, Any]:
        try:
            response = httpx.post(
                ...
                headers={
                    ...
                    "User-Agent": "ai-code-review-chat-completions-agent/0.1",
                },
                timeout=max(float(timeout_seconds), 0.001),
                verify=self.verify_tls,
            )
        except (httpx.TimeoutException, httpx.NetworkError) as exception:
            raise ChatCompletionsTransportError() from exception
        if response.status_code >= 400:
            raise ChatCompletionsTransportError(status_code=response.status_code)
        try:
            value = response.json()
        ...
        if not isinstance(value, dict):
            raise ChatCompletionsAgentError("AGENT_CHAT_COMPLETIONS_PROTOCOL_INVALID")
        return value


@dataclass(frozen=True)
class ChatCompletionsRunnerConfig:
    model: str = "synthetic-chat-model"
    ...
    def __post_init__(self) -> None:
        ...
        if not self.model.strip() or not 0 <= self.max_retries <= 3:
            raise ChatCompletionsAgentError("AGENT_CUSTOM_CONFIG_INCOMPLETE")
...
class OpenAIChatCompletionsAgentRunner:
    def __init__(
        self,
        transport: ChatCompletionsTransport,
        config: ChatCompletionsRunnerConfig | None = None,
        *,
        clock: Callable[[], float] = monotonic,
        sleeper: Callable[[float], None] = sleep,
    ) -> None:
        self.transport = transport
        self.config = config or ChatCompletionsRunnerConfig.from_budgets(
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="chat-completions-agent-review-") as name:
                temporary = Path(name)
                ...
                messages: list[dict[str, Any]] = [
                    {
                        ...
                        "content": agent_system_prompt(
                            effective_case, self.config.submit_by_turn
                        ),
                    },
                ...
                while turn_count < self.config.max_turns:
                    ...
                    response = self._request_with_retry(
                        self._request_payload(messages), cancel_event, deadline
                    )
                    turn_count += 1
                    ...
                    if not calls:
                        raise ChatCompletionsAgentError("AGENT_REVIEW_NOT_SUBMITTED")
                    if turn_count >= self.config.submit_by_turn and any(
                        call["function"]["name"] != "submit_review" for call in calls
                    ):
                        raise ChatCompletionsAgentError(
                            "AGENT_SUBMIT_DEADLINE_EXCEEDED"
                        )
                    if any(
                        call["function"]["name"] == "submit_review"
                        for call in calls[:-1]
                    ):
                        raise ChatCompletionsAgentError(
                ...
                raise ChatCompletionsAgentError("AGENT_MAX_TURNS_EXCEEDED")
        except ChatCompletionsAgentError as exception:
            return _safe_summary(
                status="FAILED",
                started=started,
                turn_count=turn_count,
            ...
            return _safe_summary(
                ...
                audit=audit,
                budgets=self.config.effective_budgets(),
                error_code="AGENT_CHAT_COMPLETIONS_PROTOCOL_INVALID",
            )
    ...
    def _request_with_retry(
        ...
        while True:
            ...
            try:
            ...
            except ChatCompletionsAgentError:
                raise
            except ChatCompletionsTransportError as exception:
                ...
                if status in RETRYABLE_STATUS_CODES and attempts < self.config.max_retries:
                    ...
                    if delay <= 0:
                        raise ChatCompletionsAgentError("AGENT_TIMEOUT") from exception
                    self.sleeper(delay)
                    continue
                raise ChatCompletionsAgentError(
    ...
    def _check_control(self, cancel_event: Any, deadline: float) -> None:
        ...
        if self.clock() >= deadline:
            raise ChatCompletionsAgentError("AGENT_TIMEOUT")
...
def _validate_response(response: Any) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for call in calls:
        if not isinstance(call, dict) or call.get("type") != "function":
            raise ChatCompletionsAgentError("AGENT_CUSTOM_TOOL_CALL_INVALID")
...
def _parse_tool_arguments(value: str) -> dict[str, Any]:
    try:
        ...
        raise ChatCompletionsAgentError("AGENT_CUSTOM_TOOL_CALL_INVALID") from exception
...
def _transport_error_code(status_code: int | None) -> str:
    ...
    if status_code == 404:
        return "AGENT_CHAT_COMPLETIONS_UNSUPPORTED"
    if status_code in {400, 409, 422}:
        return "AGENT_CHAT_COMPLETIONS_PROTOCOL_INVALID"
    if status_code == 429:
