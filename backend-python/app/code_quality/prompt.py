...
def render_instructions(request: dict[str, Any]) -> str:
    protocol = (
        ...
        "本地引用证据只表示在当前 task worktree 中检索到的有限引用片段；字段引用 snippets 也只是有限证据。不能仅凭未命中引用、引用 snippets 未注入或 requested context 不可用判定无风险，也不能覆盖硬风险。\n"
        "Context Pack 中的 notInjectedEvidence / BUDGET_CUT 表示本地已命中或已请求的证据因预算、能力或环境原因未进入模型；这不是“没有引用”或“没有风险”的证明。\n"
        "当 DTO_FIELD_CHANGED、FIELD_DELETED、METHOD_SIGNATURE_CHANGED、METHOD_DELETED 相关的关键字段引用、调用方、配置读取点或表结构证据被预算裁剪、未注入或不可用时，除非 diff 本身足以证明安全、数据一致性或线上正确性硬风险，否则 finding 必须使用 PARTIAL 或 INSUFFICIENT，并将 confidence 设为 LOW 或 MEDIUM，不能输出 HIGH confidence。\n"
