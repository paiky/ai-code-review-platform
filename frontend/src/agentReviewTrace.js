...
export function collectAgentTraceEvents(events) {
  ...
  const latestRunId = latestAgentRunId(source);
  ...
  source
    ...
    .forEach(event => {
      ...
      if (Number(detail?.runId) !== latestRunId) return;
      const terminalKey = agentTerminalPhases.has(event?.phase) ? event.phase : null;
      const key = `${detail?.runId ?? 'run'}:${terminalKey ?? detail?.sequence ?? event?.id}`;
...
export function summarizeAgentTrace(events, now = Date.now()) {
  ...
  const scoped = source.filter(event => {
    ...
    return Number(parseDetail(event?.detail)?.runId) === latestRunId;
  });
...
function latestAgentRunId(events) {
  const runIds = (Array.isArray(events) ? events : [])
    .filter(event => isAgentTraceProgressEvent(event) || isAgentHeartbeatProgressEvent(event))
    .map(event => Number(parseDetail(event?.detail)?.runId))
