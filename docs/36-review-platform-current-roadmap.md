...
### docs/39 阶段 5.5 落地记录（2026-07-10）

- 改了什么：新增任务页统一 MUI 外层壳；任务列表页头、筛选 surface 和表格容器迁移到 MUI；任务详情页头、返回 / 重新执行 / 复制重跑操作区、概要信息 surface 和 Tabs 外层容器迁移到 MUI；任务详情内部复杂 Review 面板、Diff viewer、Patch 预览、finding 展示和设置类弹窗继续保留既有实现。
- 为什么做：任务列表和任务详情是日常 Review 主入口，需要与质量治理页形成一致的 Material 3 surface、outline 和紧凑后台布局，同时保持任务筛选、分页、直达、`reviewKey` 和轮询行为稳定。
- 解决的缺口：补齐阶段 5 中普通用户主入口的高层 MUI 迁移，让质量治理页和任务页不再割裂。
- 如何验证：运行 `scripts/run-frontend.cmd build`，确认生产构建通过；确认 `/tasks`、`/tasks/{taskId}`、`/tasks/{taskId}?reviewKey=...` 路由、任务列表筛选 / 分页、详情 tab 切换、重新执行审阅、复制为新任务重跑、AI Review 轮询、高准确模式流转和确定性检查入口不回归。
- 下一阶段：`docs/39` 阶段 5.6 设置页和版本更新页 MUI 迁移；未经用户确认不继续推进。

### docs/39 阶段 5.6 落地记录（2026-07-10）

- 改了什么：复用统一 MUI 后台页面壳；设置页页头、刷新操作和 Collapse 外层容器迁移到 MUI；版本更新页页头和时间线外层容器迁移到 MUI，并把强调色调整为蓝 / 粉系；接入帮助页页头和内容外层容器迁移到 MUI；设置页内部复杂表单、表格、Provider 测试、Prompt 预览、项目组配置和 Push 策略逻辑保持不变。
- 为什么做：完成阶段 5 最后一批高层页面迁移，让任务、质量治理、设置、版本更新和接入帮助具备一致的 Material 3 外层框架，同时不重写复杂配置表单。
