...
> 状态说明：本文是多端能力的阶段推进文档。**阶段 1 / 2 已基本落地**（项目组前端管理、端类型配置、自动识别依据展示）；**阶段 3（多端 MR 拆分审查）仍未实现**。若阶段 1/2 能力已存在，请勿按 §5.1 / §5.2 重复实现。
...
## 2. 阶段 1：项目组前端管理与项目绑定（已基本完成）

### 2.1 目标

让项目组从“后端 API 可用”变成“前端可管理、可筛选、可绑定”的完整闭环。

**当前状态：** 前端“设置 -> 项目组 / 端类型配置”已支持项目组 CRUD、项目绑定与任务列表按组筛选。本节保留作验收对照；除非发现缺口，否则不要重复开发。
...
## 3. 阶段 2：自动端类型识别增强（已基本完成）

### 3.1 目标

减少 PC / iOS / Android 项目接入后的手工配置成本。新 GitLab 项目首次 webhook 进入平台时，能基于 changed files、仓库名或 namespace 给出更合理的端类型默认配置。

**当前状态：** 后端已写入 `detected_target_types` / `target_detection_json`，前端可展示识别依据并支持手动保存端类型。本节保留作验收对照与后续规则微调参考。
...
## 4. 阶段 3：多端 MR 拆分审查（未实现，当前唯一待推进阶段）
...
先从阶段 3：多端 MR 拆分审查开始；若阶段 3 尚未立项，则仅维护阶段 1/2 的使用反馈，不要重复实现项目组或端类型识别。
```
