...
`2026-08-05` 用户授权在版本更新页新增 `v1.3.0`。发布说明只概括三份已完成计划中已经实现并验证的指挥中心能力：

1. 亮色一体化指挥中心，以及顶部 5 项当前调度状态、中部 Agent / Standard 双 Review 执行轨和底部近 24 小时质量产出；
2. 左侧最近活动 ReviewTask、右侧北京时间今日 Result、真实 DOM 端口测量和六条独立连接；
3. 基于真实 queued / running / fallback 证据的线路流光、引擎双环反转和 Review 霓虹边框，以及异常、过期、移动端和 reduced-motion 静止降级。

发布说明不得声称已实现悬浮执行链、模块详情 Drawer、任务级完成抵达事件、前端推算趋势或不存在的历史 KPI。本文档状态和 M4 停止点保持不变。

验证结果：发布说明专项测试 `2 passed / 0 failed`；`scripts/run-frontend.cmd build` 通过，仅保留既有主 Chunk 大小提示；`1600×900` 浏览器确认 `v1.3.0` 位于时间轴首项、三条首屏重点完整展示且页面无横向溢出。
