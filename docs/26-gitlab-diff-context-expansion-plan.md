...
已完成。阶段 1、阶段 2、阶段 3 均已落地并完成影响范围内验证。当前停止，等待用户验收。

## 目标

让任务详情中的“查看 Diff”和“AI 修复 Patch 预览”支持按需展开完整源码上下文，并统一升级为暗黑语法高亮代码视图。
...
支持向上展开 20 行、向下展开 20 行和展开全部。查看 Diff 与 Patch 预览统一使用暗黑主题，
按文件后缀做 token 级语法高亮。完成前端 build 和浏览器验证后停止。
```
...
- 旧 MR Patch 预览如果已有 head `commitSha`，仍可读取当前源码作为基线。
