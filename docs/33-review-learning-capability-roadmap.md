...
当前 docs/32 的 V2-A 到 V2-E 已落地，V2-F-1 / V2-F-2 / V2-F-3 / V2-F-5 / V2-F-6 / V2-F-7 / V2-F-8 / V2-F-9 / V2-F-10 / V2-F-11 / V2-F-12 / V2-F-13 / V2-F-14 / V2-F-15 / V2-F-16 已落地或已验收。后续短期主线按 docs/34 进入 V2-F-17：规则缺口补全推荐算法与通用补齐流程；不要直接跳到 DB / 缓存 / MQ / 配置检索。反馈池、项目策略、上下文不足人工标记等人工沉淀能力先保留后端和数据结构，但生产前端默认屏蔽入口。更完整的自动归因统计、评估集、风险校准、自动聚类和半自动策略候选仍作为后续阶段。
...
当前不优先恢复人工沉淀产品入口，也不建议马上扩展 DB / 缓存 / MQ / 配置检索器。V2-F-14 已补 DTO / VO 字段引用检索，覆盖任务 669 这类真实高频场景；V2-F-15 已补预算裁剪与上下文完整性保护，避免已检索或已请求的证据因预算未注入时继续造成高置信误判；V2-F-16 已设计 finding 级二阶段补证据机制。下一步等待用户确认后进入 V2-F-17 规则缺口补全推荐算法与通用补齐流程。
...
V2-F-16 已完成后，不直接进入大范围 DB / 缓存 / MQ / 配置检索。下一步先推进 V2-F-17，根据规则缺口频率、影响范围、误判风险和实现成本生成补全建议，再由用户确认是否进入 V2-F-18 或其它具体 Retriever。
