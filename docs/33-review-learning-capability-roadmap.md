...
- 当前总控入口：`docs/36-review-platform-current-roadmap.md`。本文件保留 Review 学习能力长期愿景，不再作为近期阶段推进的唯一判断入口。
- 当前状态：能力愿景与后续推进计划已梳理；`docs/32` 的 V2-A 到 V2-E 已落地，V2-F-1 / V2-F-2 / V2-F-3 已落地，V2-F-5 本地仓库 mirror clone / fetch / worktree 最小闭环、V2-F-6 `METHOD_DELETED / METHOD_SIGNATURE_CHANGED` 引用搜索 Retriever MVP、V2-F-7 本地引用证据注入 Context Pack、V2-F-8 前端高准确模式摘要 / 人工沉淀入口熄灯、V2-F-9 生产验证、V2-F-10 本地 workspace 清理与磁盘保护、V2-F-11 高准确模式角色流转可观测、V2-F-12 规则缺口沉淀与优先级看板、V2-F-13 版本更新页收口、V2-F-14 DTO / VO 字段引用检索、V2-F-15 预算裁剪与上下文完整性保护、V2-F-16 Finding 级二阶段补证据设计、V2-F-17 规则缺口补全推荐算法与通用补齐流程、V2-F-18 DB / Mapper / Entity 关联检索已落地。短期主线已调整为 `docs/34-local-repository-context-retrieval-plan.md` 的本地仓库上下文检索 / 高准确 Review 模式；反馈池、项目策略、上下文不足人工标记等人工沉淀能力先作为暗能力保留，生产前端默认屏蔽。下一阶段需观察新任务 / 重跑任务的规则缺口推荐结果后再确认。
- 编写时间：2026-06-10
- 前置版本：
...
V2-F / 高准确 Review：本地仓库上下文检索
  ...
  -> 已完成：人工沉淀能力前端默认屏蔽
  -> 已完成：生产验证与效果复盘
  -> 已完成：本地 workspace 清理与磁盘保护
  -> 已完成：高准确模式角色流转可观测
  -> 已完成：规则缺口沉淀与优先级看板
  -> 已完成：版本更新页收口
  -> 已完成：DTO / VO 字段引用检索，优先降低字段变更上下文不足误判
  -> 已完成：预算裁剪与上下文完整性保护
  -> 已完成：Finding 级二阶段补证据设计
  -> 已完成：规则缺口补全推荐算法与通用补齐流程
```

说明：V2.5 的反馈自动归因和更完整上下文不足统计不取消，但后移到本地检索、预算裁剪保护和上下文完整性机制稳定之后。V2-F-13 已让裁剪和缺口可观测、可聚合，但还没有彻底解决上下文不完整导致的误判；因此下一轮应先补字段引用、高风险 signal 的预算保护和二阶段补证据，再进入评估集或更多业务检索器。
...
V2-F.5：把规则缺口聚合成补齐建议，让用户确认下一类 Planner / Retriever / 预算 / Prompt 改进
V2.5：让系统自动归因和统计反馈信号
...
当前 docs/32 的 V2-A 到 V2-E 已落地，V2-F-1 / V2-F-2 / V2-F-3 / V2-F-5 / V2-F-6 / V2-F-7 / V2-F-8 / V2-F-9 / V2-F-10 / V2-F-11 / V2-F-12 / V2-F-13 / V2-F-14 / V2-F-15 / V2-F-16 / V2-F-17 已落地或已验收。后续短期主线按 docs/34 先查看规则缺口推荐结果；不要直接跳到 DB / 缓存 / MQ / 配置检索。反馈池、项目策略、上下文不足人工标记等人工沉淀能力先保留后端和数据结构，但生产前端默认屏蔽入口。更完整的自动归因统计、评估集、风险校准、自动聚类和半自动策略候选仍作为后续阶段。
...
当前不优先恢复人工沉淀产品入口，也不建议马上扩展缓存 / MQ / 配置检索器。V2-F-14 已补 DTO / VO 字段引用检索，覆盖任务 669 这类真实高频场景；V2-F-15 已补预算裁剪与上下文完整性保护，避免已检索或已请求的证据因预算未注入时继续造成高置信误判；V2-F-16 已设计 finding 级二阶段补证据机制；V2-F-17 已把规则缺口聚合成补全建议和下一阶段 prompt 草稿；V2-F-18 已补 DB / Mapper / Entity 关联检索。下一步先观察新任务 / 重跑任务的规则缺口推荐结果，再确认是否补缓存、MQ、配置或其它具体 Retriever。
...
V2-F-18：DB / Mapper / Entity 关联检索（已完成）
```

V2-F-18 已完成后，不直接进入大范围缓存 / MQ / 配置检索。下一步先观察新任务 / 重跑任务的规则缺口推荐结果，再确认是否进入缓存、MQ、配置或其它具体 Retriever。
