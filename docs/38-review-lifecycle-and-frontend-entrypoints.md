...
- `质量治理` 是管理员入口，当前聚合质量看板、评估样本、规则缺口、验收记录和回放记录；后续按 `docs/39` 阶段 4 收敛为默认突出质量看板和评估样本，其它入口保留为高级诊断或详情入口。
- `验收记录` 和 `回放记录` 不再作为一级入口展示，但直接路由和后端记录能力保留。
...
`验收记录` 对应规则、Retriever、Prompt、Context Pack、确定性检查或 Provider 改动的治理记录，当前位于顶部导航 `质量治理` 下。
...
`回放记录` 用于保存 evaluation run / review replay run，当前位于顶部导航 `质量治理` 下。
...
`反馈池` 当前生产前端默认隐藏；启用 `VITE_REVIEW_LEARNING_UI_ENABLED=true` 后会出现在 `质量治理` 中。
