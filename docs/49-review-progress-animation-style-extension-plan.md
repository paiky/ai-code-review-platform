...
- 文档状态：已暂停，阶段一、阶段二均未开始实施。用户已决定优先规划
  `docs/50-review-running-immersive-workspace-plan.md` 的路由级沉浸工作台；未经用户在该专项验收后
  再次明确确认，不得启动本文任何阶段。
- 前置专项：`docs/48-review-task-detail-unified-progress-ui-plan.md`。
- 当前结论：现有 `BRAIN` 继续作为已上线局部 Hero 动效和后续 Canvas 故障兜底；ENERGY、Lottie、
  风格切换和浏览器偏好不进入 `docs/50`，也不因沉浸工作台实施而自动恢复。
- 历史结论：保留第一版大脑动画并扩展能量球和 Lottie 在技术上可行；若未来恢复，三种风格仍必须共享
  同一状态语义和无障碍契约，只改变表现，不改变 ReviewJourney、轮询、时间轴或 Review 结果。
...
- 不安装 Lottie 依赖，不增加 ENERGY / LOTTIE 代码、资源、菜单或 localStorage 偏好。
- 不为 `docs/50` 预先抽取动画注册表，不把本文内容夹带进沉浸工作台阶段。
- `docs/50` 使用的原生 Canvas 粒子核心是运行态工作台的固定视觉，不等同于本文的多风格切换。
- 只有 `docs/50` 完成并由用户验收后，用户再次明确要求恢复本文，才允许重新核对视觉范围、默认风格、
  性能预算和交付顺序；旧 Prompt 不构成自动恢复授权。
...
- 本文当前冻结；以下阶段说明和 Prompt 仅作为历史方案保留，不得直接执行。
...
## 16. 历史总控 Prompt（暂停，不得执行）

```text
当前计划已暂停。未经用户在 docs/50 验收后再次明确确认，不得执行以下内容。
