...
> 状态说明：本文记录多端接入的总体设计。当前产品默认按“单仓单端”使用：一个 GitLab 项目通常归属一个端类型，并通过“当前项目所属端类型”配置；`supported_target_types` / `project_target_configs` 仍保留混合仓库和多端拆分审查的扩展能力，真正按端拆分审查属于后续阶段。
